...
public class Tower extends Piece {
	...
	public boolean moveTo (Coords origin, Coords destiny, Piece[][] board) {
		...
		// CONTROL OVER RIGHT AND LEFT CANCAST
				...
				if (destiny.y == origin.y) {
					...
					if (landing(destiny, board)) {
						...
						if(swipeX(origin, destiny, board)) {
							...
							if (checkcheck(origin, destiny, board)) {
								
								if (origin.equals(new Coords(7,7)) && colour) twrightcancast = false;
								
								if (origin.equals(new Coords(0,7)) && colour) twleftcancast = false;
								
								if (origin.equals(new Coords(7,0)) && !colour) tbrightcancast = false;
								
								if (origin.equals(new Coords(0,0)) && !colour) tbleftcancast = false;
