...
public class Pawn extends Piece {
	...
	synchronized public boolean moveTo (Coords origin, Coords destiny, Piece[][] board) {
		
		System.out.println("Pawn " + origin + " is trying to make a real move");
		...
		if (destiny.x > 7 || destiny.y > 7 || destiny.x < 0 || destiny.y < 0) {
			...
			if (colour==true) {
				...
				if (origin.y-destiny.y==1 || destiny.y==4) {
					...
					if (origin.x-destiny.x==0) {
						...
						if (board[destiny.x][destiny.y]!=null) {
							...
							if (destiny.y==0) {
								...
								System.out.println("Unused block");						// Why does it enter here??
				...
				if (origin.y-destiny.y==-1  || destiny.y==3) {
